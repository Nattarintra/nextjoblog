...
**Contract**: `export type ExtendSessionState = undefined | { ok: true; extendedUntil: number } | { error: "unknown"; message: string }`. `export async function extendSession(_state: ExtendSessionState): Promise<ExtendSessionState>` — takes the previous state as its sole parameter (unused) so it matches the two-argument shape `useActionState` requires when called with no payload (`useActionState`'s dispatched action always passes `(state, ...args)`; no `FormData` is needed here since there's no form input, only an action trigger). On any Supabase error or a missing/malformed claims shape, return `{ error: "unknown", message: "..." }` and `console.error` the underlying error, mirroring `login`/`signup`'s try/catch shape. No `redirect()` — this dismisses a modal, it doesn't navigate. It also rejects on an already-lapsed current expiry (`Date.now() >= currentEffectiveExpiry`), since the app is now solely responsible for its own expiry enforcement.

**Addendum (post-implementation, recorded during impl review, 2026-09-18)**: `app/actions/session.ts` imports its claims lookup, error codes, and extension-store logic directly from `lib/session/claims.ts`, `lib/session/errors.ts`, and `lib/session/extension-store.ts` respectively (see the Phase 3 addendum below for that package's layout). It has one sibling in `app/actions/`: `session-messages.ts`, which defines `SESSION_EXTENSION_MESSAGES` (user-facing copy) and has no `lib/` counterpart. An earlier revision of this implementation added three pure re-export shim files (`session-claims.ts`, `session-errors.ts`, `session-extension-store.ts`) that only aliased the `lib/session/*` modules with no logic of their own; these were removed during impl-review triage (2026-09-18) as unnecessary indirection with no precedent elsewhere in `app/actions/`. This is a wider file layout than this contract's single-file description, but the exported action's type/signature/behavior above is unchanged.
...
**Addendum (post-implementation, recorded during impl review, 2026-09-18)**: `lib/session.ts` is now a barrel re-export over a `lib/session/` package rather than a single file: `calculations.ts` (`computeEffectiveSessionExpiresAt`, `computeNextSessionExpiry`, `isSessionExpired`), `claims.ts` (`getBaseSessionExpiresAt`, `computeBaseSessionExpiry`, `getAuthenticatedClaims`), `config.ts` (`getSessionLifetimeMs`, `getSessionNoticeWindowMs`), `errors.ts` (`SESSION_EXTENSION_ERROR_CODES`), `extension-store.ts` (the `SessionExtensionStore` interface + `createSupabaseSessionExtensionStore`, the actual `session_extensions` upsert/read), and `service.ts` (`getEffectiveSessionExpiry`). `getEffectiveSessionExpiry`'s return type is also richer than this contract's flat shape: `export type EffectiveSessionExpiry = { status: "authenticated"; claims: JwtPayload; effectiveExpiresAt: number } | { status: "lookup_error"; error: unknown }` — callers branch on `status` rather than checking for `undefined`, distinguishing "no session" from "the `session_extensions` read itself failed" (the latter is rethrown by `page.tsx` rather than treated as expired, so it fails loud rather than silently signing the user out on a transient DB error). Both the split and the richer union are accepted as sound engineering; nothing behaves differently from this contract's intent.
...
**Addendum (post-implementation, recorded during impl review, 2026-09-18)**: The actual implementation moves the `signOut()` + redirect off the Server Component render path into a new route handler, `app/api/auth/session-expired/route.ts`, with `page.tsx` calling `redirect("/api/auth/session-expired")` instead of signing out inline. This is intentional, not a bug: Server Components cannot reliably write response cookies during render, so the sign-out (which needs to clear cookies) is more correctly performed inside a route handler's own response. The route handler calls `supabase.auth.signOut()` (default `global` scope, matching this contract's original intent of a full sign-out) and redirects to `/login`.
...
**Addendum (post-implementation, recorded during impl review, 2026-09-18)**: `SessionExpiryNoticeDialog.tsx` does not call `useActionState(extendSession, undefined)` directly as this contract describes. Instead it delegates to a new hook, `app/dashboard/useSessionExtension.ts`, via `useSessionExtension({ onSuccess: onDismiss })`; that hook internally makes the exact `useActionState(extendSession, undefined)` call, and exposes `state`/`isPending` back to the dialog, plus `router.refresh()` on success. A second new hook, `app/dashboard/useDialogKeyboardNavigation.ts`, extracted the existing Tab/Shift+Tab-wrap/Escape keyboard handling out of the dialog component. Observable behavior (props surface, `aria-busy`, error display, retry, `router.refresh()` on success) matches this contract's intent; the deviation is purely in which module makes the `useActionState` call and where the keyboard logic lives. Accepted as-is — no code change.
...
**Addendum (post-implementation, recorded during impl review, 2026-09-18)**: Beyond the cycle-keyed cookie change this contract describes, `app/dashboard/session-expiry-notice.ts`'s `getNoticeState` also gained an independent `now < sessionExpiresAt` upper bound (so the notice stops being eligible once the cycle has actually expired, not just once responded), and `lib/session/config.ts` gained a new `getSessionNoticeWindowMs()` reading an optional `SESSION_NOTICE_WINDOW_MS` env var (falling back to the existing `sessionLifetimeMs / 15` ratio), replacing `layout.tsx`'s previous inline `sessionLifetimeMs / 15` computation. Neither is required by the cycle-keying work itself — both are accepted as independently-reasonable additions (a correctness fix and a configurability knob), documented here since the plan's "What We're NOT Doing" originally scoped this file to cycle-keying alone. No code change needed.
...
- [x] 5.1 `npm run test:run` passes with coverage ≥80% — 26a7613
- [x] 5.2 `npm run test:e2e` passes, including new multi-cycle and RLS tests — 26a7613

#### Manual

- [x] 5.3 Coverage report confirms new/modified files are not silently excluded — 26a7613
